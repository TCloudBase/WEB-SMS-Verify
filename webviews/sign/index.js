...
window.cloud.registerExtension(window.extSms); //装载短信扩展
loginState() //登录初始化校验

/**
 *  封装的短信发送函数对象
 *  可使用send(手机号)、 verify(验证码)、 login(验证码) 分别实现发送、校验、登录
 */
window.SMS = {
  phone: null,
  async send(phone) {
    this.phone = phone;
    try {
      return await window.cloud.invokeExtension(extSms.name, {
        action: 'Send',
        app: window.cloud,
        phone: this.phone
      });
...
function loginState() {
  ...
  auth.getLoginState().then(async (res) => {
    if (res && res._loginType == 'CUSTOM') {
      const phone = res.user.customUserId
...
async function submit() {
  ...
  submitinfo(1, '校验中')
  const result = await window.SMS.verify(code)
  console.log('校验结果：', result)
  if (result != false) {
    submitinfo(1, '提交预约中')
    const loginres = await window.SMS.login(code)
